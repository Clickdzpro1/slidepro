...
# WS14: Google genai SDK removed — Gemini image generation replaced by Prodia
# Flux Schnell via Vercel AI Gateway (OpenAI-compatible REST endpoint).
...
from utils.get_env import (
    ...
    get_prodia_flux_api_key_env,
    get_ai_gateway_base_env,
...
class ImageGenerationService:
    ...
    def get_image_gen_func(self):
        ...
        elif is_prodia_flux_selected():
            return self.generate_image_prodia_flux
    ...
    async def generate_image_prodia_flux(
        self, prompt: str, output_directory: str
    ) -> str:
        """Generate image using Prodia Flux Schnell via Vercel AI Gateway.

        WS14: replaces all Gemini image tiers (nanobanana2, nanobanana_pro,
        gemini_flash). Uses the OpenAI-compatible /v1/images/generations
        endpoint on the Vercel AI Gateway. Text-to-image only.
        ~$0.001-0.0025/image at 1024x1024 (33-67x cheaper than Gemini).

        Gateway key cascade: PRODIA_FLUX_API_KEY → CUSTOM_LLM_API_KEY.
        Gateway base: AI_GATEWAY_BASE or https://ai-gateway.vercel.sh.
        """
        api_key = get_prodia_flux_api_key_env() or os.environ.get(
            "CUSTOM_LLM_API_KEY", ""
        )
        if not api_key:
            raise HTTPException(
                status_code=503,
                detail="Flux image generation not configured. Set "
                       "PRODIA_FLUX_API_KEY or CUSTOM_LLM_API_KEY.",
            )

        gateway_base = get_ai_gateway_base_env() or "https://ai-gateway.vercel.sh"

        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{gateway_base}/v1/images/generations",
                headers={
                ...
                if resp.status != 200:
                    ...
                    raise HTTPException(
                        ...
                        detail=f"Flux image generation failed: {error_body}",
                    )
        ...
        if not b64:
            raise HTTPException(
                status_code=500,
                detail="No image generated by prodia/flux-fast-schnell",
            )
