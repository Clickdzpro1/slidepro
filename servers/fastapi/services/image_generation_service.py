...
from utils.get_env import (
    ...
    get_nanobanana2_aspect_ratio_env,
    get_nanobanana2_resolution_env,
...
class ImageGenerationService:
    ...
    def get_image_gen_func(self):
        ...
        elif is_nanobanana2_selected():
            return self.generate_image_nanobanana2
    ...
    async def generate_image_nanobanana2(
        ...
        """Generate image using NanoBanana2 (gemini-3.1-flash-image).

        The slide-optimized Gemini model: advanced text rendering for infographics/
        diagrams, 16:9 aspect ratio, up to 4K, ~11.5s latency, ~$0.067/image at 1K.
        Passes aspect_ratio + resolution via image_config (unlike the base Google
        method, which uses defaults). Defaults: 16:9, 2K — override via
        NANOBANANA2_ASPECT_RATIO and NANOBANANA2_RESOLUTION env vars.
        """
        aspect_ratio = get_nanobanana2_aspect_ratio_env() or "16:9"
        resolution = get_nanobanana2_resolution_env() or "2k"
        ...
        if not image_path:
            raise HTTPException(
                status_code=500,
                detail=f"No image generated by gemini-3.1-flash-image (nanobanana2)",
            )
